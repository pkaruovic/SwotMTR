...
/**
 * 
 * @author Andrija Klasa u kojoj se nalaze svi podaci potrebni za rad
 *         aplikacije, kao i metode kojima se ti podaci obradjuju.
 *
 *         Lista strategije sadrzi sve strategije koje korisnik kreira
 *         kombinovanjem snaga, slabosti, sansi i pretnji.
 * 
 *         Liste listaSnage, listaSlabosti, listaSanse, listaPretnje sadrze
 *         objekte klase Swot koji predstavljaju snage, slabosti, sanse i
 *         pretnje.
 */
...
public class Logika implements Serializable {
	...
	/**
	 * Metoda se koristi samo u slucaju deserijalizacije. Kada korisnik izabere
	 * datoteku u kojoj je prethodno sacuvao podatke iz ove liste, listi iz ovog
	 * objekta se dodeljuje prethodno sacuvana lista. Svi podaci koje je lista
	 * prethodno sadrzala se brisu.
	 * 
	 * @param strategije
	 *            Lista strategija koje ce se prikazivati i koje ce korisnik
	 *            koristiti.
	 */
	 ...
	 * 
	 * @param listaSnage
	 *            Lista snaga koje ce se prikazivati i koje ce korisnik
	 *            koristiti.
	 */
	 ...
	 * 
	 * @param listaSlabosti
	 *            Lista slabosti koje ce se prikazivati i koje ce korisnik
	 *            koristiti.
	 */
	 ...
	 * 
	 * @param listaSanse
	 *            Lista sansi koje ce se prikazivati i koje ce korisnik
	 *            koristiti.
	 */
	 ...
	 * 
	 * @param listaPretnje
	 *            Lista pretnji koje ce se prikazivati i koje ce korisnik
	 *            koristiti.
	 */
	...
	/**
	 * Metoda dodaje strategiju u listu strategija tako da strategije ostanu
	 * sortirane u opadajucem redosledu prema atraktivnosti kako bi korisnik
	 * mogao da ih lakse uporedjuje.
	 * 
	 * @param strategija
	 *            - objekat koji je korisnik kreirao
	 */
	 ...
	 *            - naziv snage
	 * @param ponder
	 *            - realan broj izmedju 0 i 1 koji predstavlja tezinski
	 *            koeficijent snage, odnosno, koliki znacaj ima ova snaga
	 */
	public void dodajSnaguUListu(String naziv, double ponder) {
		listaSnage.add(new Swot(naziv, ponder));
	}

	public void obrisiSnaguIzListe(String naziv) {
		for (Swot s : listaSnage) {
	 ...
	 * @param ponder
	 *            - realan broj izmedju 0 i 1 koji predstavlja tezinski
	 *            koeficijent sanse, odnosno, koliki znacaj ima ova sansa
	 */
	public void dodajSansuUListu(String naziv, double ponder) {
		listaSanse.add(new Swot(naziv, ponder));
	}

	public void obrisiSansuIzListe(String naziv) {
		for (Swot s : listaSanse) {
			if (s.getNaziv().equals(naziv)) {
				listaSanse.remove(s);
				return;
			}
	 ...
	 * @param ponder
	 *            - realan broj izmedju 0 i 1 koji predstavlja tezinski
	 *            koeficijent slabosti, odnosno, koliki znacaj ima ova slabost
	 */
	public void dodajSlabostUListu(String naziv, double ponder) {
		listaSlabosti.add(new Swot(naziv, ponder));
	}

	public void obrisiSlabostIzListe(String naziv) {
		for (Swot s : listaSlabosti) {
			if (s.getNaziv().equals(naziv)) {
				listaSlabosti.remove(s);
				return;
			}
		}
	}
	 ...
	 * @param ponder
	 *            - realan broj izmedju 0 i 1 koji predstavlja tezinski
	 *            koeficijent pretnje, odnosno, koliki znacaj ima ova pretnja
	 */
	public void dodajPretnjuUListu(String naziv, double ponder) {
		listaPretnje.add(new Swot(naziv, ponder));
	}

	public void obrisiPretnjuIzListe(String naziv) {
		for (Swot s : listaPretnje) {
			if (s.getNaziv().equals(naziv)) {
				listaPretnje.remove(s);
				return;
			}
		}
	}
	 ...
	 * 
	 * @param naziv
	 *            - naziv snage ciji ponder treba da vrati metoda
	 ...
	 * 
	 * @param naziv
	 *            - naziv sanse ciji ponder treba da vrati metoda
	 ...
	 * 
	 * @param naziv
	 *            - naziv slabosti ciji ponder treba da vrati metoda
	 ...
	 * 
	 * @param naziv
	 *            - naziv pretnje ciji ponder treba da vrati metoda
